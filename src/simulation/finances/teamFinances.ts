...
export function processFacilityUpgradeStart(
  ...
  return createTeamTransaction(
    ...
    upgradeCost,
    `${facilityName} upgrade: Level ${currentLevel} → ${nextLevel} (${upgradeDuration} weeks)`,
    week,
    year
  )
}
...
export function processManufacturingJobCost(
  ...
  return createTeamTransaction(
    ...
    totalCost,
    `In-house ${partType} manufacturing (×${quantity}) - Materials: $${materialCost.toLocaleString()}, Labor: $${laborCost.toLocaleString()}`,
    week,
    year,
    { countsTowardCostCap: true }
  )
...
export function processPartsOrderCost(
  ...
  const description = isRushOrder
    ? `Rush order: ${partType} ×${quantity} from ${manufacturerName}`
    : `Order: ${partType} ×${quantity} from ${manufacturerName}`
...
export function processShippingCost(
  ...
  const description = raceId
    ...
    : `Parts shipment (${methodName}): ${origin} → ${destination} (${partCount} parts)`
...
export function processAutoReorderCosts(
  ...
  const partsDescription = orders.map(o => `${o.partType} ×${o.quantity}`).join(', ')
