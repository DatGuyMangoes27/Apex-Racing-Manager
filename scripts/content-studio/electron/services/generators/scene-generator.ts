/**
 * Scene / atmosphere image generation tasks.
 * Replaces all Unsplash stock images with AI-generated motorsport scenes.
 * 
 * Categories:
 * - 15 racing scene categories (gt-racing, formula, etc.)
 * - 8 owner background scenes
 * - 8 hero/banner images for key screens
 * - 4 UI texture/atmosphere images
 * - 1 fallback image
 *
 * All images generated via Nano Banana Pro (gemini-3-pro-image-preview)
 * for maximum quality on these high-visibility assets.
 */
...
const IMAGE_OUTPUT_DIR = 'public/images/generated/scenes'

// ============================================================
// SCENE PROMPTS - Racing Categories
// Each category gets multiple variants for variety
// ============================================================
...
const RACING_CATEGORY_SCENES: SceneDefinition[] = [
  // GT Racing (4 variants)
  {
    id: 'gt-racing-001',
  ...
  // Prototype / Endurance (3 variants)
  {
    id: 'prototype-001',
  ...
  // Victory / Celebration (2 variants)
  {
    id: 'victory-001',
...
// ============================================================
// OWNER BACKGROUND SCENES
// ============================================================

const OWNER_BACKGROUND_SCENES: SceneDefinition[] = [
  {
    id: 'owner-bg-self-made',
  ...
  {
    id: 'owner-bg-passionate-enthusiast',
  ...
  {
    id: 'owner-bg-corporate-executive',
...
// ============================================================
// HERO / BANNER IMAGES for key screens
// ============================================================

const HERO_SCENES: SceneDefinition[] = [
  {
    id: 'hero-main-menu',
...
export function createSceneImageTasks(projectRoot: string): GenerationTask[] {
  const tasks: GenerationTask[] = []
  const outputDir = path.join(projectRoot, IMAGE_OUTPUT_DIR)
  ...
  const allScenes = [
    ...RACING_CATEGORY_SCENES,
    ...OWNER_BACKGROUND_SCENES,
    ...HERO_SCENES,
    FALLBACK_SCENE,
  ]

  for (const scene of allScenes) {
    const imagePath = path.join(projectRoot, IMAGE_OUTPUT_DIR, scene.filename)
    ...
    tasks.push({
      ...
      entityId: scene.id,
      entityName: `Scene: ${scene.id.replace(/-/g, ' ')}`,
      category: 'scenes',
      type: 'image',
      priority: 1, // Highest priority - these are the most visible images in the app
      prompt: scene.prompt,
      outputPath: imagePath,
      model: 'gemini-3-pro-image-preview', // Use Pro model for best quality
    })
  }
...
/**
 * Get the total count of scene images that need generation.
 */
export function getSceneImageCount(): number {
  return RACING_CATEGORY_SCENES.length
    + OWNER_BACKGROUND_SCENES.length
    + HERO_SCENES.length
    + 1 // fallback
}

/**
 * Get all scene definitions (useful for the stock-images data file).
 */
export function getAllSceneDefinitions() {
  return {
    racingCategories: RACING_CATEGORY_SCENES,
    ownerBackgrounds: OWNER_BACKGROUND_SCENES,
    heroes: HERO_SCENES,
