...
> **Note:** The repository also contains an older Electron/React prototype at the repo root. **v0.3 development happens in `godot-career-prototype/`** — that is the game described here.

---

## What is v0.3?

v0.3 is a major step toward a **commercial-quality AMS2 career manager**. Career creation choices permanently shape the live simulation, sponsors and finances carry real stakes into every race weekend, staff and board systems add management depth, and AMS2 integration is tighter — from install-based world import through live telemetry to byte-faithful AI export.

### Career creation → live effects
...
- **Custom AI grids** scanned from `UserData/CustomAIDrivers` — both attribute-style XML and child-element formats (NAMeS, official grids).
- **Per-class opt-in** with driver/livery counts and ~60 class alias mappings (`F-Classic_Gen1` → Formula Classic G1, `MiniChallenge` → JCW, etc.).
- **Livery-aware driver assignment** — imported drivers match their AMS2 livery first; skill ratings merge into career ratings.
...
- **Contracts** — negotiate accept/lowball, fire with severance, fill vacancies.
...
- **Board meetings** — exitable, negotiable objectives, per-series mandates.
- **Dynamic prize-money objectives** from the real payout ladder.
- **Credit line & bailouts** — three missed loan payments trigger board intervention with lasting commercial reputation damage.
...
- **Full transaction ledger** — every cash movement recorded with running balance on the Finances screen.
- **Weekly operations costs** scale with fleet and real staff wages.
- **Loans** — draw against commercial profile; weekly servicing with strike-and-bailout paths.
- **Race Day stakes panel** — guaranteed fees, win/podium bonuses, attendance clauses, and deals near termination before you run the weekend.

### Nothing is instant

Major actions flow through a **pending-actions pipeline** — accept sponsors (1–2 days), series entry (2–3 days), car delivery (3–7 days), staff offers (1–3 days), workshop locks (1–7 days), loan approval (1–2 days). Confirmations arrive by mail; advance time with **Continue** to resolve them.

### Calendar, test days & race loop

- Calendar starts **January 5, 2026** (no snap to first race).
- **Test days** — book from Calendar, drive in AMS2 or simulate; track knowledge, shakedown findings, wear, and debrief mail.
- **Multi-race weekends** for Sprint championships — independent qualifying per race, combined weekend scoring.
- **Regulation enforcement** — mandatory pit stops void points when telemetry confirms no stop.
...
**Not** Documents — the install path only.

- Byte-faithful to NAMeS / Ultimate Companion XML format (BOM, CRLF, exact element order).
- **Stats-only changes** — NAMeS driver names are preserved; career world seeds from install XML ground truth.
- Enable **"Customized AI Names"** in AMS2: **Options → Gameplay**.
...
- Two-part maintenance — automatic weekend running cost plus manual per-system or full service in the garage.
- Buy-now with delivery delay; auction wins deliver in 2–5 days; cars show **IN TRANSIT** until arrival.

### Support & diagnostics

- Persistent file logging with session header (version, engine, OS, timestamp); logs rotate automatically.
- **Settings → Support** — live AMS2 telemetry status, Open Logs Folder, Report an Issue (GitHub).
...
1. **Create a career** — import your AMS2 world, pick team DNA, staff, and starting championship(s).
2. **Build your programme** — enter series, hire staff, sign sponsors, buy cars, book test days.
3. **Advance the calendar** — resolve mail, pending actions, board meetings, and auctions between race weeks.
4. **Race in AMS2** — Apex Racing Manager reads live telemetry; AI export syncs the field before you load the session.
5. **Consequences apply** — points, prize money, sponsor satisfaction, wear, finances, and inbox stories update automatically.
...
3. Press **F5** (or Play) — main scene is `res://scenes/intro.tscn`.
...
```
Carrer Mod/
├── godot-career-prototype/          # ← Active v0.3 game
│   ├── scenes/                      # UI scenes (intro, home, race day, staff, …)
│   ├── scripts/
│   │   ├── prototype_state.gd     # Core career state & pending actions
│   │   ├── career_creation_flow.gd
│   │   ├── adapters/                # AMS2 import/export, career creation
│   │   ├── dev/                     # Headless smoke tests
│   │   └── *_screen.gd              # Screen controllers
│   ├── data/                        # Series, tracks, sponsors, fixtures
│   └── project.godot                # config/version = 0.3.0
├── electron/                        # Legacy Electron prototype (not v0.3)
└── src/                             # Legacy React UI (not v0.3)
```
...
| Known | Driver-change rules are advisory — only pit stops are telemetry-verified today. |
